...
describes what users get. Pick a format with `-o`, or let nvd choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
nvd <command> -o table   # aligned columns for reading
nvd <command> -o jsonl   # one JSON object per line, for piping
nvd <command> -o json    # a single JSON array
nvd <command> -o csv     # spreadsheet friendly
nvd <command> -o tsv     # tab-separated
nvd <command> -o url     # just the URL column
nvd <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
nvd <command> --fields id,title,url
```
...
```bash
nvd <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
nvd <command>            # a table, because this is a terminal
nvd <command> | wc -l    # JSONL, because this is a pipe
```
