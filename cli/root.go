// Package cli assembles the nvd command tree from the nvd
...
// NewApp assembles the kit application from the nvd domain. The
...
//
// To add a command, declare it in nvd/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := nvd.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(nvd.Domain{}).Register(app)
